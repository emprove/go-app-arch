...
func NewRouter(cfg *config.Cfg, ds *config.DynamicState, db database.DB) http.Handler {
	...
	productAdmSrv := service.NewProductAdmService(ds, productRepo)

	productHandlerAdmReg(rrAdm, productAdmSrv)

	return rr
}

func productHandlerAdmReg(rr *routegroup.Bundle, sp service.ProductAdmService) {
